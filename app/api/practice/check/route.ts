...
export async function POST(request: Request) {
  ...
  if (typeof questionId !== "string" || !Number.isInteger(answerIndex)) return NextResponse.json({ error: "Invalid answer" }, { status: 400 });
  const question = PRACTICE_QUESTIONS.find((item) => item.id === questionId);
  const answer = PRACTICE_ANSWER_KEY[questionId];
  if (!question || !answer || Number(answerIndex) < 0 || Number(answerIndex) >= question.choices.length) return NextResponse.json({ error: "Invalid answer" }, { status: 400 });
