...
The learner profile combines recent diagnostic and practice evidence by skill. It distinguishes accurate-and-on-pace strengths, rushing errors, slow-but-accurate work, slow-and-inaccurate work, and knowledge gaps. The highest-priority signal selects the next practice skill, behavioral focus, and starting difficulty, while the progress page explains the same evidence in plain language with accessible drill links.
...
Practice uses short exposure-aware sets drawn from a 357-item original drill bank, bringing the full app inventory to 427 questions. The bank includes substantial contextual sentence completion, exact-pair comparison, advanced multi-step math, logic, and spatial coverage across difficulty levels 1–5. Learners can launch dedicated drills for every available question family. The latest expansions strengthen vocabulary, algebraic relationships, digit algebra, exponents, geometry, mixtures, numerical analogies, quadratic reasoning, systems of equations, work rates, coding rules, spatial tracking, matrices, and selectable verbal and pattern practice. Completed sets retain question IDs and skill-level outcomes, allowing the next set to favor unseen items and prioritize the latest diagnostic weakness or the skill with the strongest recent miss signal. Previously seen items return only as the rotating bank is exhausted.
