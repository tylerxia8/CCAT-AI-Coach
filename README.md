...
Practice uses short exposure-aware sets drawn from a 337-item original drill bank, bringing the full app inventory to 407 questions. The bank includes substantial contextual sentence completion, exact-pair comparison, advanced multi-step math, logic, and spatial coverage across difficulty levels 1–5. Learners can also launch dedicated 10-question drills for verbal analogies, antonyms, and number sequences. The latest expansions strengthen coding rules, classification and mixed series, cube relations, spatial tracking, reflection, rotation, matrices, paper folding, and selectable verbal and pattern practice. Completed sets retain question IDs and skill-level outcomes, allowing the next set to favor unseen items and prioritize the latest diagnostic weakness or the skill with the strongest recent miss signal. Previously seen items return only as the rotating bank is exhausted.
