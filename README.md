...
Practice uses short exposure-aware sets drawn from a 382-item original drill bank, bringing the full app inventory to 452 questions. The bank includes substantial contextual sentence completion, exact-pair comparison, advanced multi-step math, logic, and spatial coverage across difficulty levels 1–5. Learners can launch dedicated drills for every available question family. The latest expansions strengthen vocabulary and analogies, charts and tables, exponent and geometry work, numerical analogies, quadratic reasoning, systems of equations, coding rules, mixed series, classification, rotation, reflection, visual sequences, and matrices. Completed sets retain question IDs and skill-level outcomes, allowing the next set to favor unseen items and prioritize the latest diagnostic weakness or the skill with the strongest recent miss signal. Previously seen items return only as the rotating bank is exhausted.
