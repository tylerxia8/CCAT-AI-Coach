...
import { parseTransferQueue, scheduleTransfer, TRANSFER_QUEUE_KEY } from "@/lib/transfer-store";
import { INTERVENTION_HISTORY_KEY, parseInterventionRuns, treatmentFor, treatmentLabel, type InstructionTreatment } from "@/lib/intervention-experiments";
...
export function RemediationLessonExperience() {
  ...
  const [treatment, setTreatment] = useState<InstructionTreatment>("worked_then_problem");
  useEffect(() => {
    ...
    const targetSkill = requested && /^[a-z0-9 &-]{2,40}$/i.test(requested) ? requested : "deductive reasoning";
    setTreatment(treatmentFor(targetSkill, parseInterventionRuns(window.localStorage.getItem(INTERVENTION_HISTORY_KEY))));
  }, []);
  ...
  function answer(moduleIndex: number, choiceIndex: number) {
    ...
    if (next[skill].length === lesson.modules.length) {
      const transfers = parseTransferQueue(window.localStorage.getItem(TRANSFER_QUEUE_KEY));
      window.localStorage.setItem(TRANSFER_QUEUE_KEY, JSON.stringify(scheduleTransfer(transfers, skill)));
      const runs = parseInterventionRuns(window.localStorage.getItem(INTERVENTION_HISTORY_KEY));
      window.localStorage.setItem(INTERVENTION_HISTORY_KEY, JSON.stringify([...runs, { skill, treatment, completedAt: new Date().toISOString() }].slice(-50)));
    }
  }
  ...
  return <main className="lesson-shell">
    ...
    <section className={`lesson-transfer ${completed.length === lesson.modules.length ? "ready" : ""}`}><div><div className="section-label">Final step</div><h2>{completed.length === lesson.modules.length ? "Now recognize, automate, and transfer the method." : "Pass each check before timed practice."}</h2><p>{completed.length === lesson.modules.length ? "A delayed unseen check will return in three days so short-term familiarity is not mistaken for mastery." : `${lesson.modules.length - completed.length} lesson check${lesson.modules.length - completed.length === 1 ? " remains" : "s remain"}.`}</p></div>{completed.length === lesson.modules.length && <div className="lesson-transfer-actions"><Link className="primary link-button" href={`/recognition?skill=${encodeURIComponent(skill)}`}>Recognize the method →</Link><Link className="secondary link-button" href={`/practice?focus=knowledge&skill=${encodeURIComponent(skill)}&intervention=${treatment}&new=1`}>Start transfer drill</Link></div>}</section>
  </main>;
}
