...
export function ResultSummary({ result }: { result: ScoredDiagnosticResult }) {
  ...
  const misconception = result.diagnosis.misconceptions[0];
  ...
  return (
    <section className="result-summary" aria-label="Your three-part result summary">
      ...
      <article><span>Most likely reason</span><strong>{misconception?.label ?? causeLabel(result.diagnosis.primaryCause)}</strong><p>{misconception ? `${misconception.evidence} ${misconception.repair}` : result.diagnosis.summary}</p></article>
