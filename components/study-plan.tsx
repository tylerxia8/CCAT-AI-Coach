...
export function StudyPlanExperience() {
  ...
  return (
    <main className="plan-shell">
      ...
      <section className="plan-list">{plan.sessions.map((session) => { const isComplete = state.completedSessionIds.includes(session.id); return <article className={isComplete ? "completed" : ""} key={session.id}><button aria-label={`${isComplete ? "Mark incomplete" : "Mark complete"}: ${session.title}`} onClick={() => toggle(session.id)}>{isComplete ? "✓" : ""}</button><div className="plan-day">Session {session.day}<small>{session.minutes} min</small></div><div><h3>{session.title}</h3><p>{session.purpose}</p></div><Link href={session.href}>{session.actionLabel} →</Link></article>; })}</section>
    </main>
  );
}

function PlanNav() {
  return <nav className="nav"><Link className="brand brand-link" href="/"><span>AC</span>Aptitude Coach</Link><div className="nav-actions"><Link className="nav-text-link" href="/progress">Progress</Link><Link className="nav-link" href="/auth">Account</Link></div></nav>;
}
